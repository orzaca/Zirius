...
$sql = "SELECT p.id, COALESCE(up.content, p.content) AS content FROM pagina8 p  
        LEFT JOIN user_pagina8 up ON p.id = up.pagina8_id AND up.user_id = :userId";
...
<body>
    ...
    <div id="editModal8"> <!-- Cambiado a editModal3 -->
        <div class="modal-content">
            <span class="close-btn" onclick="closeEditModal()">&times;</span>
            <h2>Editar Guion</h2>
            <textarea id="editText8" rows="4" style="width: 100%;"></textarea> <!-- Cambiado a editText3 -->
            <button id="saveChanges8" onclick="saveParagraph()">Guardar</button> <!-- Cambiado a saveChanges3  -->
        </div>
    </div>
...
/* */       const textElement = document.getElementById('para8' + paraId).getElementsByTagName('p')[0]; 
 ...
 /* */      const updatedText = document.getElementById('editText8').value; 
 /* */      const textElement = document.getElementById('para8' + currentParaId).getElementsByTagName('p')[0]; 
            textElement.innerText = updatedText; 

            // Envía la nueva información al servidor
/* */       fetch('update_pagina8.php', { 
                method: 'POST',
                headers: {
